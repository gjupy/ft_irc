...
Client::Client(int fd, Server &server)
    : m_fd(fd), m_is_registered(false), _server(server), m_username(""),
...
void Client::handle_exit(const std::string &buffer) {
  if (!m_is_registered)
    throw std::invalid_argument(
        RED "451 You have not registered to the server\r\n" RESET);
  ...
  if (channel == NULL)
    throw std::invalid_argument(RED "403 " + channel_name +
                                ": No such channel" RESET);
  if (!is_member(channel->get_registered(), m_nickname))
    throw std::invalid_argument(RED "441 " + channel_name +
                                ": Your aren't on that channel" RESET);
  handle_privmsg(m_nickname + " you left channel " + channel_name + "\r\n");
  handle_privmsg(channel_name + " " + m_nickname + " left channel " +
                 channel_name + "\r\n");
...
void Client::add_user(map_str_str &channels_to_keys) {
  ...
  map_channels::iterator it_channel;
  for (map_str_str::iterator it = channels_to_keys.begin();
       it != channels_to_keys.end(); ++it) {
    if ((it_channel = channels.find(it->first)) !=
        channels.end()) // if channels exists
    {
      if (is_registered_channel(it_channel->second))
        throw std::invalid_argument(RED "888 " + it_channel->first +
                                    ": You're already on that channel" RESET);
      if (it_channel->second->get_user_limit() &&
          it_channel->second->get_registered().size() >=
              it_channel->second->get_user_limit())
        throw std::invalid_argument(RED "471 " + it_channel->first +
                                    ": Cannot join channel (+l)" RESET);
      if (it_channel->second->get_invite_only() &&
          !is_invited(it_channel->second, m_nickname))
        throw std::invalid_argument(RED "473 " + it_channel->first +
                                    ": Cannot join channel (+i)" RESET);
      if (it_channel->second->get_key_needed() &&
          !is_valid_key(it_channel->second, it->second))
        throw std::invalid_argument(RED "475 " + it_channel->first +
                                    ": Cannot join channel (+k)" RESET);
      it_channel->second->set_registered(*this);
      _server.send_to_client(m_nickname, BLUE "you joined channel " +
                                             it->first + "\r\n" RESET);
      handle_privmsg(it->first + " " + m_nickname + " joined channel " +
                     it->first + "\r\n");
    } else {
      ...
      _server.send_to_client(m_nickname, BLUE "you created channel " +
                                             it->first + "\r\n" RESET);
    }
  }
}
...
void Client::join_parser(const std::string &buffer,
  ...
  for (map_str_str::iterator it = channels_to_keys.begin();
    ...
    if (it->first[0] != '#' || it->first.length() == 1 ||
      ...
      throw std::invalid_argument(RED "410 " + it->first +
                                  ": Erroneous channel name" RESET);
  }
  try {
...
void Client::handle_join(const std::string &buffer) {
  if (!m_is_registered)
    throw std::invalid_argument(
        RED "451 You have not registered to the server" RESET);
...
void Client::parse_command(const std::string &command) {
  ...
  std::cerr << "Error\n"
            << "invalid input\n";
  _server.send_to_client(m_nickname,
                         RED "421 " + cmd + ": Unknown command\r\n" RESET);
}
...
void Client::handle_user(const std::string &args) {
  if (!m_authenticated)
    throw std::invalid_argument(
        RED "451 Please provide a server password using PASS command" RESET);
  if (!m_username.empty())
    throw std::invalid_argument(
        RED "462 Unauthorized command (already registered)" RESET);
  if (args.empty())
    throw std::invalid_argument(RED "461 USER: Not enough parameters" RESET);
  if (username_exists(args))
    throw std::invalid_argument(RED "433 " + args +
                                ": This username is already in use" RESET);
  ...
  if (arg[0] == '#' || arg == "*")
    throw std::invalid_argument(RED "438 " + arg +
...
void Client::handle_nick(const std::string &args) {
  if (!m_authenticated || m_username.empty())
    throw std::invalid_argument(
        RED "451 You need to register using PASS and USER commands" RESET);
  ...
  if (arg[0] == '#')
    throw std::invalid_argument(RED "432 " + arg +
                                ": Erroneous nickname" RESET);
  m_nickname = arg;
  m_is_registered = true;
  std::cout << "Client " << m_fd << " set nickname to: " << m_nickname
            << std::endl;
  _server.send_to_client(m_nickname,
                         GREEN "001 " + m_nickname +
                             ": Welcome to the Internet Relay Network, " +
                             m_nickname + "!\r\n" RESET); // 001: RPL_WELCOME
  _server.send_to_client(
      m_nickname,
      GREEN "002 " + m_nickname + ": Your host is " + std::string(HOST) +
          ", running version IRC_TRASH\r\n" RESET); // 002: RPL_YOURHOST
}
...
void Client::invite_client(Client &client, Channel &channel) {
  ...
  _server.send_to_client(m_nickname, BLUE "341 you invited " +
                                         client.get_nickname() + " to join " +
                                         channel.get_name() + "\r\n" RESET);
  _server.send_to_client(client.m_nickname,
                         BLUE "341 you were invited by " + m_nickname +
                             " to join " + channel.get_name() + "\r\n" RESET);
}
...
void Client::handle_mode(const std::string &buffer) {
  ...
  if (input_channel == NULL)
    throw std::invalid_argument(RED "403 " + channel_name +
                                ": No such channel" RESET);
  if (mode[0] != '+' && mode[0] != '-')
    throw std::invalid_argument(RED "472 " + mode +
                                ": is not a valid mode" RESET);
  if (is_operator(*input_channel, m_nickname) == false)
    throw std::invalid_argument(RED "482 " + channel_name +
                                ": You're not channel operator" RESET);
  if (mode.empty()) {
    _server.send_to_client(m_nickname,
                           BLUE "324 " + channel_name + ": modes: " +
  ...
  case ('o'):
    if (mode_param.empty())
      throw std::invalid_argument(RED "461 MODE: Not enough parameters" RESET);
    if (input_client == NULL)
      throw std::invalid_argument(RED "406 " + mode_param +
                                  ": There was no such nickname" RESET);
    if (is_member(input_channel->get_registered(), mode_param) == false)
      throw std::invalid_argument(RED "441 " + mode_param + ": " +
                                  channel_name +
                                  " :You aren't on that channel" RESET);
    if (mode[0] == '+') {
      if (is_operator(*input_channel, mode_param))
        throw std::invalid_argument(RED "666 " + mode_param + ": " +
                                    channel_name +
                                    ": is already a channel operator" RESET);
      input_channel->set_operator(mode_param, give);
    } else
      input_channel->set_operator(mode_param, take);
    handle_privmsg(input_channel->get_name() + " " + mode_param + " was made " +
                   mode[0] + "o by " + m_nickname);
    break;
  case ('l'):
    if (mode_param.empty())
      throw std::invalid_argument(RED "461 MODE: Not enough parameters" RESET);
    if (mode[0] == '+') {
      ...
      if (limit <= 0)
        throw std::invalid_argument(
            RED "696 " + str_limit +
            ": is not a valid limit (must be > 0)" RESET);
...
void Client::kick_user(map_str_str &channels_to_nick,
  ...
  for (map_str_str::iterator it = channels_to_nick.begin();
    ...
    if (input_channel == NULL)
      throw std::invalid_argument(RED "403 " + it->first +
                                  ": No such channel" RESET);
    Client *input_client = find_client(clients, it->second);
    if (input_client == NULL)
      throw std::invalid_argument(RED "406 " + it->second +
                                  ": There was no such nickname" RESET);
    if (!is_member(input_channel->get_registered(), it->second))
      throw std::invalid_argument(RED "441 " + it->second + ": " + it->first +
                                  ": They aren't on that channel" RESET);
    if (!is_operator(*input_channel, m_nickname))
      throw std::invalid_argument(RED "482 " + it->first +
                                  ": You're not channel operator" RESET);
    handle_privmsg(it->second + " you were kicked out of channel " + it->first +
                   "\r\n");
    handle_privmsg(it->first + " " + it->second +
                   " got kicked out of channel " + it->first + "\r\n");
...
void Client::handle_topic(const std::string &buffer) {
  ...
  if (topic_name.empty()) {
    ...
    if (topic.empty())
      throw std::invalid_argument(RED "331 " + channel_name +
                                  ": No topic is set" RESET);
    else
      _server.send_to_client(m_nickname, BLUE "332 " + channel_name +
                                             ": topic: " + topic +
                                             std::string("\r\n") + "" RESET);
    ...
    handle_privmsg(channel_name + " " + channel_name + ": " + m_nickname +
                   " removed the channel's topic\r\n");
    input_channel->set_topic("");
  } else {
    handle_privmsg(channel_name + " " + channel_name + ": " + m_nickname +
                   " set new channel topic: " + topic_name + "\r\n");
